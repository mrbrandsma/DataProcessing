...
# Store movies from csv file in a dictionary
with open(INPUT_CSV, newline='') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        movie = (row['Title'], row['Rating'], row['Year'], row['Actors'], row['Runtime'])
        movie_list.append(movie)

# Global dictionary for the data
data_dict = {str(key): [] for key in range(START_YEAR, END_YEAR)}

# Add movie ratings to the list of the correct year
for item in movie_list:
    movie_rating = float(item[1])
    movie_year = item[2]

    # Add movie rating to correct year
    data_dict[movie_year].append(movie_rating)

# Calculate averages for every year
avg_movie_ratings = {}
for year,rating in data_dict.items():
    avg_movie_ratings[year] = sum(rating)/ len(rating)

# Create a plot that shows average ratings of 2008-2017
